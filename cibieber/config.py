...
def execute_test(sha1, name):
    ...
    cmd = './docker_with_timeout.sh 240m ' \
          '--net none ' \
          '-e QUIET=1 ' \
          '-e hash={0} ' \
          '-e name={1} ' \
          '-e BUILDS=\'clang\' ' \
          '-v "$(pwd)/dealii:/home/bob/source:ro" ' \
          '-v "$(pwd)/logs/{0}:/home/bob/log" ' \
          'tjhei/dealii-tester-alpha'.format(sha1, name)
    print "cmd:",cmd
    try:
        answer = subprocess.check_output(cmd,
                                         shell=True,stderr=subprocess.STDOUT)
...
def make_link(sha):
    return "http://www.math.clemson.edu/~heister/cib/dealii-tester-alpha/{}/".format(sha)
